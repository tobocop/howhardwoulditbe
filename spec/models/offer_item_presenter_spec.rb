...
describe OfferItemPresenter do
  ...
  let(:presenter) { OfferItemPresenter.new(offer, virtual_currency: virtual_currency, view_context: view_context) }
  ...
  describe 'call_to_action_link' do
    it 'returns the add to my wallet text if the first condition is true' do
      ...
      presenter.call_to_action_link(true, false)
    end

    it 'returns the Link Your Card text if the 2nd condition is true' do
      ...
      presenter.call_to_action_link(false, true)
    end

    it 'returns the Join Plink Today text if both conditions are false' do
      ...
      presenter.call_to_action_link(false, false)
    end
  end
