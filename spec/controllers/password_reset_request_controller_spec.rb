...
describe PasswordResetRequestController do
  describe 'POST create' do
    it 'redirect to the home page and set a flash message if email is found' do
      PasswordResetRequestForm.stub(:new).with({'email' => 'mail@example.com'}).and_return(stub(:fake_password_reset, save: true))
    ...
    it 're-renders the form if the reset cannot be sent' do
      PasswordResetRequestForm.stub(:new).with({'email' => 'mail@example.com'}).and_return(stub(:fake_password_reset, save: false))
